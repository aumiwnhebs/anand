.class public Lcom/download/library/DownloadTask;
.super Lcom/download/library/Extra;
.source "SourceFile"

# interfaces
.implements Ljava/io/Serializable;
.implements Ljava/lang/Cloneable;


# static fields
.field public static final STATUS_CANCELED:I = 0x3ee

.field public static final STATUS_DOWNLOADING:I = 0x3ea

.field public static final STATUS_ERROR:I = 0x3ef

.field public static final STATUS_NEW:I = 0x3e8

.field public static final STATUS_PAUSED:I = 0x3ec

.field public static final STATUS_PAUSING:I = 0x3eb

.field public static final STATUS_PENDDING:I = 0x3e9

.field public static final STATUS_SUCCESSFUL:I = 0x3ed

.field static final TAG:Ljava/lang/String;


# instance fields
.field protected authority:Ljava/lang/String;

.field beginTime:J

.field connectTimes:I

.field detalTime:J

.field endTime:J

.field volatile isAWait:Z

.field isCustomFile:Z

.field volatile loaded:J

.field mCondition:Ljava/util/concurrent/locks/Condition;

.field protected mContext:Landroid/content/Context;

.field protected mDownloadListener:Lcom/download/library/e;

.field protected mDownloadNotifier:Lcom/download/library/m;

.field mDownloadStatusListener:Lcom/download/library/n;

.field protected mDownloadingListener:Lcom/download/library/q;

.field protected mFile:Ljava/io/File;

.field mId:I

.field mThrowable:Ljava/lang/Throwable;

.field mTotalsLength:J

.field mutex:Ljava/util/concurrent/locks/Lock;

.field pauseTime:J

.field redirect:Ljava/lang/String;

.field private volatile status:I

.field uniquePath:Z


# direct methods
.method static constructor <clinit>()V
    .locals 2

    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    const-string v1, "Download-"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-class v1, Lcom/download/library/DownloadTask;

    invoke-virtual {v1}, Ljava/lang/Class;->getSimpleName()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    sput-object v0, Lcom/download/library/DownloadTask;->TAG:Ljava/lang/String;

    return-void
.end method

.method public constructor <init>()V
    .locals 5

    invoke-direct {p0}, Lcom/download/library/Extra;-><init>()V

    invoke-static {}, Lcom/download/library/x;->y()Lcom/download/library/x;

    move-result-object v0

    invoke-virtual {v0}, Lcom/download/library/x;->i()I

    move-result v0

    iput v0, p0, Lcom/download/library/DownloadTask;->mId:I

    const-string v0, ""

    iput-object v0, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    const-wide/16 v1, 0x0

    iput-wide v1, p0, Lcom/download/library/DownloadTask;->beginTime:J

    iput-wide v1, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    iput-wide v1, p0, Lcom/download/library/DownloadTask;->endTime:J

    iput-wide v1, p0, Lcom/download/library/DownloadTask;->detalTime:J

    const/4 v3, 0x0

    iput-boolean v3, p0, Lcom/download/library/DownloadTask;->isCustomFile:Z

    const/4 v4, 0x1

    iput-boolean v4, p0, Lcom/download/library/DownloadTask;->uniquePath:Z

    iput v3, p0, Lcom/download/library/DownloadTask;->connectTimes:I

    iput-wide v1, p0, Lcom/download/library/DownloadTask;->loaded:J

    iput-object v0, p0, Lcom/download/library/DownloadTask;->redirect:Ljava/lang/String;

    const/4 v0, 0x0

    iput-object v0, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    iput-object v0, p0, Lcom/download/library/DownloadTask;->mCondition:Ljava/util/concurrent/locks/Condition;

    iput-boolean v3, p0, Lcom/download/library/DownloadTask;->isAWait:Z

    const/16 v0, 0x3e8

    iput v0, p0, Lcom/download/library/DownloadTask;->status:I

    return-void
.end method

.method private checkCustomFilePath(Ljava/io/File;)V
    .locals 3

    const/4 v0, 0x0

    if-eqz p1, :cond_2

    invoke-virtual {p1}, Ljava/io/File;->getAbsolutePath()Ljava/lang/String;

    move-result-object p1

    invoke-static {}, Lcom/download/library/x;->y()Lcom/download/library/x;

    move-result-object v1

    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getContext()Landroid/content/Context;

    move-result-object v2

    invoke-virtual {v1, v2}, Lcom/download/library/x;->r(Landroid/content/Context;)Ljava/io/File;

    move-result-object v1

    invoke-virtual {v1}, Ljava/io/File;->getAbsolutePath()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {p1, v1}, Ljava/lang/String;->startsWith(Ljava/lang/String;)Z

    move-result p1

    if-eqz p1, :cond_0

    goto :goto_1

    :cond_0
    iget-object p1, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    invoke-static {p1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result p1

    const/4 v1, 0x1

    if-nez p1, :cond_1

    invoke-virtual {p0, v1}, Lcom/download/library/DownloadTask;->setEnableIndicator(Z)Lcom/download/library/DownloadTask;

    :goto_0
    iput-boolean v1, p0, Lcom/download/library/DownloadTask;->isCustomFile:Z

    goto :goto_2

    :cond_1
    invoke-virtual {p0, v0}, Lcom/download/library/DownloadTask;->setEnableIndicator(Z)Lcom/download/library/DownloadTask;

    goto :goto_0

    :cond_2
    :goto_1
    iput-boolean v0, p0, Lcom/download/library/DownloadTask;->isCustomFile:Z

    :goto_2
    return-void
.end method


# virtual methods
.method protected addHeader(Ljava/lang/String;Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 1

    iget-object v0, p0, Lcom/download/library/Extra;->mHeaders:Ljava/util/HashMap;

    if-nez v0, :cond_0

    new-instance v0, Ljava/util/HashMap;

    invoke-direct {v0}, Ljava/util/HashMap;-><init>()V

    iput-object v0, p0, Lcom/download/library/Extra;->mHeaders:Ljava/util/HashMap;

    :cond_0
    iget-object v0, p0, Lcom/download/library/Extra;->mHeaders:Ljava/util/HashMap;

    invoke-virtual {v0, p1, p2}, Ljava/util/HashMap;->put(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    return-object p0
.end method

.method anotify()V
    .locals 2

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    if-nez v0, :cond_0

    return-void

    :cond_0
    invoke-interface {v0}, Ljava/util/concurrent/locks/Lock;->lock()V

    :try_start_0
    iget-object v0, p0, Lcom/download/library/DownloadTask;->mCondition:Ljava/util/concurrent/locks/Condition;

    invoke-interface {v0}, Ljava/util/concurrent/locks/Condition;->signalAll()V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    invoke-interface {v0}, Ljava/util/concurrent/locks/Lock;->unlock()V

    return-void

    :catchall_0
    move-exception v0

    iget-object v1, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    invoke-interface {v1}, Ljava/util/concurrent/locks/Lock;->unlock()V

    throw v0
.end method

.method protected autoOpenIgnoreMD5()Lcom/download/library/DownloadTask;
    .locals 3

    const/4 v0, 0x1

    iput-boolean v0, p0, Lcom/download/library/Extra;->mAutoOpen:Z

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    if-eqz v0, :cond_0

    iget-object v0, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-eqz v0, :cond_0

    invoke-static {}, Lcom/download/library/x;->y()Lcom/download/library/x;

    move-result-object v0

    sget-object v1, Lcom/download/library/DownloadTask;->TAG:Ljava/lang/String;

    const-string v2, "Custom file path, you must specify authority, otherwise the auto open should be closed. "

    invoke-virtual {v0, v1, v2}, Lcom/download/library/x;->F(Ljava/lang/String;Ljava/lang/String;)V

    const/4 v0, 0x0

    iput-boolean v0, p0, Lcom/download/library/Extra;->mAutoOpen:Z

    :cond_0
    return-object p0
.end method

.method protected autoOpenWithMD5(Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 4

    invoke-static {p1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-eqz v0, :cond_0

    return-object p0

    :cond_0
    const/4 v0, 0x1

    iput-boolean v0, p0, Lcom/download/library/Extra;->mAutoOpen:Z

    iget-object v1, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    if-eqz v1, :cond_1

    iget-object v1, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    invoke-static {v1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v1

    if-eqz v1, :cond_1

    invoke-static {}, Lcom/download/library/x;->y()Lcom/download/library/x;

    move-result-object v1

    sget-object v2, Lcom/download/library/DownloadTask;->TAG:Ljava/lang/String;

    const-string v3, "Custom file path, you must specify authority, otherwise the auto open should be closed. "

    invoke-virtual {v1, v2, v3}, Lcom/download/library/x;->F(Ljava/lang/String;Ljava/lang/String;)V

    const/4 v1, 0x0

    iput-boolean v1, p0, Lcom/download/library/Extra;->mAutoOpen:Z

    :cond_1
    iput-object p1, p0, Lcom/download/library/Extra;->targetCompareMD5:Ljava/lang/String;

    iput-boolean v0, p0, Lcom/download/library/Extra;->calculateMD5:Z

    return-object p0
.end method

.method await()V
    .locals 3

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    if-nez v0, :cond_0

    return-void

    :cond_0
    invoke-interface {v0}, Ljava/util/concurrent/locks/Lock;->lock()V

    :goto_0
    const/4 v0, 0x0

    :try_start_0
    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->isCompleted()Z

    move-result v1

    if-nez v1, :cond_1

    const/4 v1, 0x1

    iput-boolean v1, p0, Lcom/download/library/DownloadTask;->isAWait:Z

    iget-object v1, p0, Lcom/download/library/DownloadTask;->mCondition:Ljava/util/concurrent/locks/Condition;

    invoke-interface {v1}, Ljava/util/concurrent/locks/Condition;->await()V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    goto :goto_0

    :catchall_0
    move-exception v1

    goto :goto_1

    :cond_1
    iget-object v1, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    invoke-interface {v1}, Ljava/util/concurrent/locks/Lock;->unlock()V

    iput-boolean v0, p0, Lcom/download/library/DownloadTask;->isAWait:Z

    return-void

    :goto_1
    iget-object v2, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    invoke-interface {v2}, Ljava/util/concurrent/locks/Lock;->unlock()V

    iput-boolean v0, p0, Lcom/download/library/DownloadTask;->isAWait:Z

    throw v1
.end method

.method protected cancel()V
    .locals 2

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v0

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->endTime:J

    const/16 v0, 0x3ee

    invoke-virtual {p0, v0}, Lcom/download/library/DownloadTask;->setStatus(I)V

    return-void
.end method

.method public clone()Lcom/download/library/DownloadTask;
    .locals 1

    .line 1
    :try_start_0
    new-instance v0, Lcom/download/library/DownloadTask;

    invoke-direct {v0}, Lcom/download/library/DownloadTask;-><init>()V

    invoke-virtual {p0, v0}, Lcom/download/library/Extra;->copy(Lcom/download/library/Extra;)Lcom/download/library/Extra;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    return-object v0

    :catchall_0
    move-exception v0

    invoke-virtual {v0}, Ljava/lang/Throwable;->printStackTrace()V

    new-instance v0, Lcom/download/library/DownloadTask;

    invoke-direct {v0}, Lcom/download/library/DownloadTask;-><init>()V

    return-object v0
.end method

.method public bridge synthetic clone()Ljava/lang/Object;
    .locals 1

    .line 2
    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->clone()Lcom/download/library/DownloadTask;

    move-result-object v0

    return-object v0
.end method

.method protected closeAutoOpen()Lcom/download/library/DownloadTask;
    .locals 1

    const/4 v0, 0x0

    iput-boolean v0, p0, Lcom/download/library/Extra;->mAutoOpen:Z

    return-object p0
.end method

.method protected completed()V
    .locals 2

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v0

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->endTime:J

    return-void
.end method

.method protected createNotifier()V
    .locals 3

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mDownloadNotifier:Lcom/download/library/m;

    if-eqz v0, :cond_0

    invoke-virtual {v0, p0}, Lcom/download/library/m;->C(Lcom/download/library/DownloadTask;)V

    goto :goto_0

    :cond_0
    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getContext()Landroid/content/Context;

    move-result-object v0

    invoke-virtual {v0}, Landroid/content/Context;->getApplicationContext()Landroid/content/Context;

    move-result-object v0

    if-eqz v0, :cond_1

    invoke-virtual {p0}, Lcom/download/library/Extra;->isEnableIndicator()Z

    move-result v1

    if-eqz v1, :cond_1

    new-instance v1, Lcom/download/library/m;

    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getId()I

    move-result v2

    invoke-direct {v1, v0, v2}, Lcom/download/library/m;-><init>(Landroid/content/Context;I)V

    iput-object v1, p0, Lcom/download/library/DownloadTask;->mDownloadNotifier:Lcom/download/library/m;

    invoke-virtual {v1, p0}, Lcom/download/library/m;->C(Lcom/download/library/DownloadTask;)V

    :cond_1
    :goto_0
    iget-object v0, p0, Lcom/download/library/DownloadTask;->mDownloadNotifier:Lcom/download/library/m;

    if-eqz v0, :cond_2

    invoke-virtual {v0}, Lcom/download/library/m;->H()V

    :cond_2
    return-void
.end method

.method protected destroy()V
    .locals 5

    const/4 v0, -0x1

    iput v0, p0, Lcom/download/library/DownloadTask;->mId:I

    const/4 v0, 0x0

    iput-object v0, p0, Lcom/download/library/Extra;->mUrl:Ljava/lang/String;

    iput-object v0, p0, Lcom/download/library/DownloadTask;->mContext:Landroid/content/Context;

    iput-object v0, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    const/4 v1, 0x0

    iput-boolean v1, p0, Lcom/download/library/Extra;->mIsForceDownload:Z

    const/4 v2, 0x1

    iput-boolean v2, p0, Lcom/download/library/Extra;->mEnableIndicator:Z

    const v3, 0x1080081

    iput v3, p0, Lcom/download/library/Extra;->mDownloadIcon:I

    const v3, 0x1080082

    iput v3, p0, Lcom/download/library/Extra;->mDownloadDoneIcon:I

    iput-boolean v2, p0, Lcom/download/library/Extra;->mIsParallelDownload:Z

    iput-boolean v2, p0, Lcom/download/library/Extra;->mIsBreakPointDownload:Z

    const-string v2, ""

    iput-object v2, p0, Lcom/download/library/Extra;->mUserAgent:Ljava/lang/String;

    iput-object v2, p0, Lcom/download/library/Extra;->mContentDisposition:Ljava/lang/String;

    iput-object v2, p0, Lcom/download/library/Extra;->mMimetype:Ljava/lang/String;

    const-wide/16 v3, -0x1

    iput-wide v3, p0, Lcom/download/library/Extra;->mContentLength:J

    iget-object v3, p0, Lcom/download/library/Extra;->mHeaders:Ljava/util/HashMap;

    if-eqz v3, :cond_0

    invoke-virtual {v3}, Ljava/util/HashMap;->clear()V

    iput-object v0, p0, Lcom/download/library/Extra;->mHeaders:Ljava/util/HashMap;

    :cond_0
    const/4 v0, 0x3

    iput v0, p0, Lcom/download/library/Extra;->retry:I

    iput-object v2, p0, Lcom/download/library/Extra;->fileMD5:Ljava/lang/String;

    iput-object v2, p0, Lcom/download/library/Extra;->targetCompareMD5:Ljava/lang/String;

    iput-boolean v1, p0, Lcom/download/library/Extra;->calculateMD5:Z

    return-void
.end method

.method protected error()V
    .locals 2

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v0

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->endTime:J

    const/16 v0, 0x3ef

    invoke-virtual {p0, v0}, Lcom/download/library/DownloadTask;->setStatus(I)V

    return-void
.end method

.method getAuthority()Ljava/lang/String;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    return-object v0
.end method

.method public getBeginTime()J
    .locals 2

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->beginTime:J

    return-wide v0
.end method

.method public getContext()Landroid/content/Context;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mContext:Landroid/content/Context;

    return-object v0
.end method

.method public getDownloadListener()Lcom/download/library/e;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mDownloadListener:Lcom/download/library/e;

    return-object v0
.end method

.method public getDownloadStatusListener()Lcom/download/library/n;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mDownloadStatusListener:Lcom/download/library/n;

    return-object v0
.end method

.method getDownloadingListener()Lcom/download/library/q;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mDownloadingListener:Lcom/download/library/q;

    return-object v0
.end method

.method public getFile()Ljava/io/File;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    return-object v0
.end method

.method public getFileMD5()Ljava/lang/String;
    .locals 2

    iget-object v0, p0, Lcom/download/library/Extra;->fileMD5:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-eqz v0, :cond_0

    invoke-static {}, Lcom/download/library/x;->y()Lcom/download/library/x;

    move-result-object v0

    iget-object v1, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    invoke-virtual {v0, v1}, Lcom/download/library/x;->G(Ljava/io/File;)Ljava/lang/String;

    move-result-object v0

    iput-object v0, p0, Lcom/download/library/Extra;->fileMD5:Ljava/lang/String;

    if-nez v0, :cond_0

    const-string v0, ""

    iput-object v0, p0, Lcom/download/library/Extra;->fileMD5:Ljava/lang/String;

    :cond_0
    invoke-super {p0}, Lcom/download/library/Extra;->getFileMD5()Ljava/lang/String;

    move-result-object v0

    return-object v0
.end method

.method public getFileUri()Landroid/net/Uri;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    invoke-static {v0}, Landroid/net/Uri;->fromFile(Ljava/io/File;)Landroid/net/Uri;

    move-result-object v0

    return-object v0
.end method

.method public getId()I
    .locals 1

    iget v0, p0, Lcom/download/library/DownloadTask;->mId:I

    return v0
.end method

.method public getLoaded()J
    .locals 2

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->loaded:J

    return-wide v0
.end method

.method getRedirect()Ljava/lang/String;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->redirect:Ljava/lang/String;

    return-object v0
.end method

.method public declared-synchronized getStatus()I
    .locals 1

    monitor-enter p0

    :try_start_0
    iget v0, p0, Lcom/download/library/DownloadTask;->status:I
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit p0

    return v0

    :catchall_0
    move-exception v0

    monitor-exit p0

    throw v0
.end method

.method getThrowable()Ljava/lang/Throwable;
    .locals 1

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mThrowable:Ljava/lang/Throwable;

    return-object v0
.end method

.method public getTotalsLength()J
    .locals 2

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->mTotalsLength:J

    return-wide v0
.end method

.method public getUsedTime()J
    .locals 5

    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3ea

    const-wide/16 v2, 0x0

    if-ne v0, v1, :cond_1

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->beginTime:J

    cmp-long v4, v0, v2

    if-lez v4, :cond_0

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v0

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->beginTime:J

    sub-long/2addr v0, v2

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->detalTime:J

    sub-long v2, v0, v2

    :cond_0
    return-wide v2

    :cond_1
    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3ee

    if-ne v0, v1, :cond_3

    :cond_2
    :goto_0
    iget-wide v0, p0, Lcom/download/library/DownloadTask;->endTime:J

    :goto_1
    iget-wide v2, p0, Lcom/download/library/DownloadTask;->beginTime:J

    sub-long/2addr v0, v2

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->detalTime:J

    sub-long/2addr v0, v2

    return-wide v0

    :cond_3
    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3e9

    if-ne v0, v1, :cond_5

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    cmp-long v4, v0, v2

    if-lez v4, :cond_4

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->beginTime:J

    sub-long/2addr v0, v2

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->detalTime:J

    sub-long v2, v0, v2

    :cond_4
    return-wide v2

    :cond_5
    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3ec

    if-eq v0, v1, :cond_a

    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3eb

    if-ne v0, v1, :cond_6

    goto :goto_2

    :cond_6
    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3e8

    if-ne v0, v1, :cond_8

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    cmp-long v4, v0, v2

    if-lez v4, :cond_7

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->beginTime:J

    sub-long/2addr v0, v2

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->detalTime:J

    sub-long v2, v0, v2

    :cond_7
    return-wide v2

    :cond_8
    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3ed

    if-eq v0, v1, :cond_2

    iget v0, p0, Lcom/download/library/DownloadTask;->status:I

    const/16 v1, 0x3ef

    if-ne v0, v1, :cond_9

    goto :goto_0

    :cond_9
    return-wide v2

    :cond_a
    :goto_2
    iget-wide v0, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    goto :goto_1
.end method

.method public isCanceled()Z
    .locals 2

    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getStatus()I

    move-result v0

    const/16 v1, 0x3ee

    if-ne v0, v1, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method isCompleted()Z
    .locals 2

    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getStatus()I

    move-result v0

    const/16 v1, 0x3ee

    if-eq v0, v1, :cond_1

    const/16 v1, 0x3ec

    if-eq v0, v1, :cond_1

    const/16 v1, 0x3ed

    if-eq v0, v1, :cond_1

    const/16 v1, 0x3ef

    if-ne v0, v1, :cond_0

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    goto :goto_1

    :cond_1
    :goto_0
    const/4 v0, 0x1

    :goto_1
    return v0
.end method

.method isCustomFile()Z
    .locals 1

    iget-boolean v0, p0, Lcom/download/library/DownloadTask;->isCustomFile:Z

    return v0
.end method

.method isDataURI()Z
    .locals 2

    iget-object v0, p0, Lcom/download/library/Extra;->mUrl:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_0

    iget-object v0, p0, Lcom/download/library/Extra;->mUrl:Ljava/lang/String;

    const-string v1, "data"

    invoke-virtual {v0, v1}, Ljava/lang/String;->startsWith(Ljava/lang/String;)Z

    move-result v0

    if-eqz v0, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method public isPaused()Z
    .locals 2

    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getStatus()I

    move-result v0

    const/16 v1, 0x3ec

    if-ne v0, v1, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method public isPausing()Z
    .locals 2

    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getStatus()I

    move-result v0

    const/16 v1, 0x3eb

    if-ne v0, v1, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method isSuccessful()Z
    .locals 2

    invoke-virtual {p0}, Lcom/download/library/DownloadTask;->getStatus()I

    move-result v0

    const/16 v1, 0x3ed

    if-ne v0, v1, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method public isUniquePath()Z
    .locals 1

    iget-boolean v0, p0, Lcom/download/library/DownloadTask;->uniquePath:Z

    return v0
.end method

.method protected pause()V
    .locals 2

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v0

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    const/4 v0, 0x0

    iput v0, p0, Lcom/download/library/DownloadTask;->connectTimes:I

    const/16 v0, 0x3ec

    invoke-virtual {p0, v0}, Lcom/download/library/DownloadTask;->setStatus(I)V

    return-void
.end method

.method public pausing()V
    .locals 2

    const/16 v0, 0x3eb

    invoke-virtual {p0, v0}, Lcom/download/library/DownloadTask;->setStatus(I)V

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v0

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    return-void
.end method

.method resetConnectTimes()V
    .locals 1

    const/4 v0, 0x0

    iput v0, p0, Lcom/download/library/DownloadTask;->connectTimes:I

    return-void
.end method

.method resetTime()V
    .locals 2

    const-wide/16 v0, 0x0

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->beginTime:J

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->endTime:J

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->detalTime:J

    return-void
.end method

.method protected setBlockMaxTime(J)Lcom/download/library/DownloadTask;
    .locals 0

    iput-wide p1, p0, Lcom/download/library/Extra;->blockMaxTime:J

    return-object p0
.end method

.method protected setBreakPointDownload(Z)Lcom/download/library/DownloadTask;
    .locals 0

    iput-boolean p1, p0, Lcom/download/library/Extra;->mIsBreakPointDownload:Z

    return-object p0
.end method

.method protected setCalculateMD5(Z)V
    .locals 0

    iput-boolean p1, p0, Lcom/download/library/Extra;->calculateMD5:Z

    return-void
.end method

.method protected setConnectTimeOut(J)Lcom/download/library/DownloadTask;
    .locals 0

    iput-wide p1, p0, Lcom/download/library/Extra;->connectTimeOut:J

    return-object p0
.end method

.method setContentDisposition(Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/Extra;->mContentDisposition:Ljava/lang/String;

    return-object p0
.end method

.method setContentLength(J)Lcom/download/library/DownloadTask;
    .locals 0

    iput-wide p1, p0, Lcom/download/library/Extra;->mContentLength:J

    return-object p0
.end method

.method protected setContext(Landroid/content/Context;)Lcom/download/library/DownloadTask;
    .locals 0

    invoke-virtual {p1}, Landroid/content/Context;->getApplicationContext()Landroid/content/Context;

    move-result-object p1

    iput-object p1, p0, Lcom/download/library/DownloadTask;->mContext:Landroid/content/Context;

    return-object p0
.end method

.method protected setDownloadDoneIcon(I)Lcom/download/library/DownloadTask;
    .locals 0

    iput p1, p0, Lcom/download/library/Extra;->mDownloadDoneIcon:I

    return-object p0
.end method

.method protected setDownloadListener(Lcom/download/library/e;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/DownloadTask;->mDownloadListener:Lcom/download/library/e;

    return-object p0
.end method

.method protected setDownloadListenerAdapter(Lcom/download/library/f;)Lcom/download/library/DownloadTask;
    .locals 0

    invoke-virtual {p0, p1}, Lcom/download/library/DownloadTask;->setDownloadListener(Lcom/download/library/e;)Lcom/download/library/DownloadTask;

    invoke-virtual {p0, p1}, Lcom/download/library/DownloadTask;->setDownloadingListener(Lcom/download/library/q;)Lcom/download/library/DownloadTask;

    invoke-virtual {p0, p1}, Lcom/download/library/DownloadTask;->setDownloadStatusListener(Lcom/download/library/n;)V

    return-object p0
.end method

.method setDownloadStatusListener(Lcom/download/library/n;)V
    .locals 0

    iput-object p1, p0, Lcom/download/library/DownloadTask;->mDownloadStatusListener:Lcom/download/library/n;

    return-void
.end method

.method protected setDownloadTimeOut(J)Lcom/download/library/DownloadTask;
    .locals 0

    iput-wide p1, p0, Lcom/download/library/Extra;->downloadTimeOut:J

    return-object p0
.end method

.method protected setDownloadingListener(Lcom/download/library/q;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/DownloadTask;->mDownloadingListener:Lcom/download/library/q;

    return-object p0
.end method

.method protected setEnableIndicator(Z)Lcom/download/library/DownloadTask;
    .locals 2

    if-eqz p1, :cond_0

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    if-eqz v0, :cond_0

    iget-object v0, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-eqz v0, :cond_0

    invoke-static {}, Lcom/download/library/x;->y()Lcom/download/library/x;

    move-result-object p1

    sget-object v0, Lcom/download/library/DownloadTask;->TAG:Ljava/lang/String;

    const-string v1, "Custom file path, you must specify authority, otherwise the notification should not be turned on. "

    invoke-virtual {p1, v0, v1}, Lcom/download/library/x;->F(Ljava/lang/String;Ljava/lang/String;)V

    const/4 p1, 0x0

    :cond_0
    iput-boolean p1, p0, Lcom/download/library/Extra;->mEnableIndicator:Z

    return-object p0
.end method

.method protected setFile(Ljava/io/File;)Lcom/download/library/DownloadTask;
    .locals 1

    .line 1
    iput-object p1, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    const-string v0, ""

    iput-object v0, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    invoke-direct {p0, p1}, Lcom/download/library/DownloadTask;->checkCustomFilePath(Ljava/io/File;)V

    return-object p0
.end method

.method protected setFile(Ljava/io/File;Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 1

    .line 2
    invoke-virtual {p1}, Ljava/io/File;->exists()Z

    move-result v0

    if-nez v0, :cond_0

    invoke-virtual {p1}, Ljava/io/File;->isFile()Z

    move-result v0

    if-eqz v0, :cond_0

    :try_start_0
    invoke-virtual {p1}, Ljava/io/File;->createNewFile()Z
    :try_end_0
    .catch Ljava/io/IOException; {:try_start_0 .. :try_end_0} :catch_0

    goto :goto_0

    :catch_0
    move-exception p1

    invoke-virtual {p1}, Ljava/lang/Throwable;->printStackTrace()V

    invoke-static {}, Lcom/download/library/x;->y()Lcom/download/library/x;

    move-result-object p1

    sget-object p2, Lcom/download/library/DownloadTask;->TAG:Ljava/lang/String;

    const-string v0, "create file error ."

    invoke-virtual {p1, p2, v0}, Lcom/download/library/x;->F(Ljava/lang/String;Ljava/lang/String;)V

    return-object p0

    :cond_0
    :goto_0
    iput-object p1, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    iput-object p2, p0, Lcom/download/library/DownloadTask;->authority:Ljava/lang/String;

    invoke-direct {p0, p1}, Lcom/download/library/DownloadTask;->checkCustomFilePath(Ljava/io/File;)V

    return-object p0
.end method

.method setFileMD5(Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/Extra;->fileMD5:Ljava/lang/String;

    return-object p0
.end method

.method setFileSafe(Ljava/io/File;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/DownloadTask;->mFile:Ljava/io/File;

    return-object p0
.end method

.method protected setForceDownload(Z)Lcom/download/library/DownloadTask;
    .locals 0

    iput-boolean p1, p0, Lcom/download/library/Extra;->mIsForceDownload:Z

    return-object p0
.end method

.method protected setIcon(I)Lcom/download/library/DownloadTask;
    .locals 0

    iput p1, p0, Lcom/download/library/Extra;->mDownloadIcon:I

    return-object p0
.end method

.method setLoaded(J)V
    .locals 0

    iput-wide p1, p0, Lcom/download/library/DownloadTask;->loaded:J

    return-void
.end method

.method setMimetype(Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/Extra;->mMimetype:Ljava/lang/String;

    return-object p0
.end method

.method protected setParallelDownload(Z)Lcom/download/library/DownloadTask;
    .locals 0

    iput-boolean p1, p0, Lcom/download/library/Extra;->mIsParallelDownload:Z

    return-object p0
.end method

.method protected setQuickProgress(Z)Lcom/download/library/DownloadTask;
    .locals 0

    iput-boolean p1, p0, Lcom/download/library/Extra;->quickProgress:Z

    return-object p0
.end method

.method setRedirect(Ljava/lang/String;)V
    .locals 0

    iput-object p1, p0, Lcom/download/library/DownloadTask;->redirect:Ljava/lang/String;

    return-void
.end method

.method protected setRetry(I)Lcom/download/library/DownloadTask;
    .locals 1

    const/4 v0, 0x5

    if-le p1, v0, :cond_0

    const/4 p1, 0x5

    :cond_0
    if-gez p1, :cond_1

    const/4 p1, 0x0

    :cond_1
    iput p1, p0, Lcom/download/library/Extra;->retry:I

    return-object p0
.end method

.method declared-synchronized setStatus(I)V
    .locals 3
    .param p1    # I
        .annotation build Lcom/download/library/DownloadTask$DownloadTaskStatus;
        .end annotation
    .end param

    monitor-enter p0

    :try_start_0
    iput p1, p0, Lcom/download/library/DownloadTask;->status:I

    iget-object v0, p0, Lcom/download/library/DownloadTask;->mDownloadStatusListener:Lcom/download/library/n;

    if-eqz v0, :cond_0

    invoke-static {}, Lt4/e;->a()Lt4/d;

    move-result-object v1

    new-instance v2, Lcom/download/library/DownloadTask$a;

    invoke-direct {v2, p0, v0, p0, p1}, Lcom/download/library/DownloadTask$a;-><init>(Lcom/download/library/DownloadTask;Lcom/download/library/n;Lcom/download/library/DownloadTask;I)V

    invoke-virtual {v1, v2}, Lt4/d;->j(Ljava/lang/Runnable;)V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    goto :goto_0

    :catchall_0
    move-exception p1

    goto :goto_1

    :cond_0
    :goto_0
    monitor-exit p0

    return-void

    :goto_1
    monitor-exit p0

    throw p1
.end method

.method protected setTargetCompareMD5(Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/Extra;->targetCompareMD5:Ljava/lang/String;

    invoke-static {p1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result p1

    if-nez p1, :cond_0

    const/4 p1, 0x1

    iput-boolean p1, p0, Lcom/download/library/Extra;->calculateMD5:Z

    :cond_0
    return-object p0
.end method

.method setThrowable(Ljava/lang/Throwable;)V
    .locals 0

    iput-object p1, p0, Lcom/download/library/DownloadTask;->mThrowable:Ljava/lang/Throwable;

    return-void
.end method

.method setTotalsLength(J)V
    .locals 0

    iput-wide p1, p0, Lcom/download/library/DownloadTask;->mTotalsLength:J

    return-void
.end method

.method protected setUniquePath(Z)V
    .locals 0

    iput-boolean p1, p0, Lcom/download/library/DownloadTask;->uniquePath:Z

    return-void
.end method

.method protected setUrl(Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/Extra;->mUrl:Ljava/lang/String;

    return-object p0
.end method

.method protected setUserAgent(Ljava/lang/String;)Lcom/download/library/DownloadTask;
    .locals 0

    iput-object p1, p0, Lcom/download/library/Extra;->mUserAgent:Ljava/lang/String;

    return-object p0
.end method

.method protected declared-synchronized setup()V
    .locals 1

    monitor-enter p0

    :try_start_0
    iget-object v0, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    if-nez v0, :cond_0

    new-instance v0, Ljava/util/concurrent/locks/ReentrantLock;

    invoke-direct {v0}, Ljava/util/concurrent/locks/ReentrantLock;-><init>()V

    iput-object v0, p0, Lcom/download/library/DownloadTask;->mutex:Ljava/util/concurrent/locks/Lock;

    invoke-interface {v0}, Ljava/util/concurrent/locks/Lock;->newCondition()Ljava/util/concurrent/locks/Condition;

    move-result-object v0

    iput-object v0, p0, Lcom/download/library/DownloadTask;->mCondition:Ljava/util/concurrent/locks/Condition;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    goto :goto_0

    :catchall_0
    move-exception v0

    goto :goto_1

    :cond_0
    :goto_0
    monitor-exit p0

    return-void

    :goto_1
    monitor-exit p0

    throw v0
.end method

.method protected successful()V
    .locals 2

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v0

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->endTime:J

    const/16 v0, 0x3ed

    invoke-virtual {p0, v0}, Lcom/download/library/DownloadTask;->setStatus(I)V

    return-void
.end method

.method updateTime(J)V
    .locals 5

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->beginTime:J

    const-wide/16 v2, 0x0

    cmp-long v4, v0, v2

    if-nez v4, :cond_0

    iput-wide p1, p0, Lcom/download/library/DownloadTask;->beginTime:J

    return-void

    :cond_0
    cmp-long v2, v0, p1

    if-eqz v2, :cond_1

    iget-wide v0, p0, Lcom/download/library/DownloadTask;->detalTime:J

    iget-wide v2, p0, Lcom/download/library/DownloadTask;->pauseTime:J

    sub-long/2addr p1, v2

    invoke-static {p1, p2}, Ljava/lang/Math;->abs(J)J

    move-result-wide p1

    add-long/2addr v0, p1

    iput-wide v0, p0, Lcom/download/library/DownloadTask;->detalTime:J

    :cond_1
    return-void
.end method
